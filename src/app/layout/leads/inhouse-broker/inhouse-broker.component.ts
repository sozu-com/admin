...
export class InhouseBrokerComponent implements OnInit {
  ...
  bulkAssign() {
    // this.assign.keyword = '';
    if(this.user.data.permissions.can_in_house_broker == 1 || this.user.permissions.can_cordinator == 1){
  ...
  openAddChangeStatusModel(item){
    ...
    this.admin.getApi("leads/all-in-house-broker-statuses" ).subscribe(r => {
      ...
      this.spinner.hide();
      if(item && (this.user.data.permissions.can_in_house_broker == 1 || this.users.permissions.can_csr_coordinator == 1)){
