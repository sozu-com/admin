...
export class ForSaleComponent implements OnInit {
  ...
  public openPropertyConfigModal(template: TemplateRef<any>, id, meta_title_en, meta_title_es, meta_description_en, meta_description_es, type, status) {
    //  this.parameter.index = index;
    this.property.sale_tag.id = id;
    this.property.sale_tag.meta_title_en = meta_title_en;
    this.property.sale_tag.meta_title_es = meta_title_es == null ? meta_title_en : meta_title_es;
    this.property.sale_tag.meta_description_en = meta_description_en;
    this.property.sale_tag.meta_description_es = meta_description_es;
    this.property.sale_tag.block_status = status;
  ...
  getConfigurations() {
    this.spinner.show();
    this.parameter.url = 'getSaleMetatag';
    const input = new FormData();
    this.admin.postDataApi(this.parameter.url, { hide_blocked: 0 })
      .subscribe(
        success => {
          this.spinner.hide();
          this.parameter.sales = success.data;
  ...
  deletePropertyConfiguration = (item: any, index: number): void => {
    this.spinner.show();
    this.admin.postDataApi('deleteSaleMetatag', { id: item.id }).subscribe((response) => {
      this.spinner.hide();
      if (response.success == '0') {
        ...
        this.parameter.sales.splice(index, 1);
      }
    }, (error) => {
      this.spinner.hide();
