...
@Component({
  selector: 'app-mortage-loans',
  templateUrl: './mortage-loans.component.html',
  styleUrls: ['./mortage-loans.component.css'],
  providers: [Constant, Property]
})
export class MortageLoansComponent implements OnInit {
  ...
  public openPropertyConfigModal(template: TemplateRef<any>, id, meta_title_en, meta_title_es, meta_description_en, meta_description_es) {
    ...
    this.property.loan_tag.id = id;
    this.property.loan_tag.meta_title_en = meta_title_en;
    this.property.loan_tag.meta_title_es = meta_title_es == null ? meta_title_en : meta_title_es;
    this.property.loan_tag.meta_description_en = meta_description_en;
    this.property.loan_tag.meta_description_es = meta_description_es;
    //this.property.loan_tag.block_status = status;
        ...
        .subscribe(
          success => {
            ...
            if (this.parameter.index !== -1) {
              ...
              this.parameter.loans.push(success.data);
            }
            this.parameter.index = -1;
          }, error => {
            this.spinner.hide();
  ...
  getConfigurations() {
    this.spinner.show();
    this.parameter.url = 'getLoanMetatag';
    const input = new FormData();
    this.admin.postDataApi(this.parameter.url, { hide_blocked: 0 })
      .subscribe(
        success => {
          this.spinner.hide();
          this.parameter.loans = success.data;
  ...
  deletePropertyConfiguration = (item: any, index: number): void => {
    this.spinner.show();
    this.admin.postDataApi('deleteLoanMetatag', { id: item.id }).subscribe((response) => {
      this.spinner.hide();
      if (response.success == '0') {
        ...
        this.parameter.loans.splice(index, 1);
      }
    }, (error) => {
      this.spinner.hide();
