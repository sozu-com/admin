    ...
    if (this.exportfinalData) {
      ...
      for (let index = 0; index < this.exportfinalData.length; index++) {
        ...
        exportfinalData.push({
          ...
          'List Price': this.getTransformedAmount(p.min_price || 0),//parseInt(p.min_price) || 0,
          'Final Price': this.getTransformedAmount(p.final_price || 0), //parseInt(p.final_price) || 0,
          'Carpet Area': parseInt(p.max_area) || 0,
          'Agent Commission (in %)': this.getTransformedAmount(p.broker_commision || 0),//parseInt(p.broker_commision) || 0,
          'Commercialized by SOZU': p.is_commercialized ? 'yes' : 'no',
          'Total Commission (in %)': this.getTransformedAmount(p.total_commission || 0),//parseInt(p.total_commission) || 0,
