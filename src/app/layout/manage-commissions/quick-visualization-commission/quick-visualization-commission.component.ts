...
export class QuickVisualizationCommissionComponent implements OnInit {
  ...
  agentOut_payment_sum: any;
  sumData:any;
  agent_minus:any;
  agentoutside_minus:any;
    ...
    this.admin.postDataApi('getComissionFinalAmount', { id: this.property_collection_id })
      .subscribe(
        success => {
          ...
          this.agentOut_payment_sum = this.sumData.outside_agent;
      })
  }
  ...
  getoutAgentInfo(item: any,ind) {
    this.outside_agent_payment = [];
    if (this.visibleIndex === ind) {
      this.visibleIndex = -1;
    } else {
      this.visibleIndex = ind;
    ...
    this.collectionCommission.forEach((r) => {
      if (item == (r.payment_choice || {}).id) {
        for (let i = 0; i < (r.outside_agent_payment || []).length; i++) {
          ...
          this.outside_agent_payment.push(paymntsss);
        }
      }
    });
    this.isShown = ! this.isShown;
  ...
  purchase() {
    this.model.collection_commissions.forEach((r) => {
      console.log(r.purchase_payment, "vkheo")
      this.pur = r.purchase_payment;
      if (r.purchase_payment) {
