...
export class AddOfficeComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    this.model.office_contributors_param = [];
    this.model.office_contributors = [];
    this.name = '';
      ...
      if (this.id) {
        ...
        this.admin.postDataApi('getOfficeById', { office_id: this.id }).subscribe(r => {
          ...
          r.data.office_contributors.forEach(element => {
            self.model.office_contributors_param.push({
          ...
          this.model.office_tower_edit_index = '-1';
          this.model.floors = 0;
          ...
          this.projectLogo.image = this.model.office_logo;
          this.model.configurations.map((item) => {
            item.office_configuration_id = item.id;
          });
          ...
          this.admin.postDataApi('getOfficeAmenities', { hide_blocked: 1, language: lang == 'en' ? 1 : 2 }).subscribe(res => {
            ...
            if (this.model.office_towers && this.model.office_towers.length > 0) {
              // setting true to tower selected amenities
              this.model.office_towers.map(item => {
              ...
              this.model.office_towers.forEach(element => {
        ...
        this.admin.postDataApi('getBuildingRequest', { building_request_id: params.request_id }).subscribe(r => {
          ...
          if (r.data.developer == null) {
            ...
            this.model.office_request_id = params.request_id;
    ...
    this.getCountries('');
    this.initForm();
    this.admin.postDataApi('getOfficePossessionStatuses', { hide_blocked: 1 }).subscribe(r => {
      this.all_possession_statuses = r.data;
    });
    this.admin.postDataApi('getOfficeBuildingTypes', { hide_blocked: 1 }).subscribe(r => {
  ...
  addProject() {
    ...
    modelSave.office_logo = this.projectLogo.image;
    ...
    if (modelSave.images) {
      modelSave.office_images = modelSave.images.map(r => r.image);
    }
    ...
    modelSave.office_towers = this.model.office_towers;
    if (modelSave.office_towers && modelSave.office_towers.length > 0) {
      modelSave.office_towers.forEach(element1 => {
    ...
    const isBuildingTowerDetailsAvailable = this.buildingTowerDetailsAvailable(modelSave.office_towers)
    ...
    if (
      ...
      modelSave.name && modelSave.country_id && modelSave.state_id &&
      modelSave.city_id && modelSave.locality_id && modelSave.address && modelSave.address != null &&
      modelSave.lat && modelSave.lng && modelSave.office_type_id && modelSave.num_of_properties
      ...
      (modelSave.office_towers || []).length > 0 && isBuildingTowerDetailsAvailable
    ) {
    ...
    if (
      ...
      //modelSave.document && 
      modelSave.office_type_id && modelSave.num_of_properties
      && modelSave.description && modelSave.description != null
      && modelSave.possession_status_id &&
      //modelSave.launch_date && 
      isAnyAmenitiesCheck &&
      (modelSave.amenities || []).length > 0 && (modelSave.configurations || []).length > 0 && (modelSave.office_towers || []).length > 0 &&
    ...
    if (this.model.office_request_id) {
      modelSave.office_request_id = this.model.office_request_id;
    }

    if (this.id) {
      modelSave.office_id = this.id;
      ...
      this.spinner.show();
      this.admin.postDataApi('updateOffice', modelSave).subscribe(success => {
        this.spinner.hide();
      ...
      delete modelSave.id;
      delete modelSave.office_id;
      this.spinner.show();
      this.admin.postDataApi('addOffice', modelSave).subscribe(success => {
        this.spinner.hide();
  ...
  setProjectModel(data) {
    ...
    self.model.office_contributors_param = self.model.office_contributors_param ? self.model.office_contributors_param : [];
    self.model.office_contributors = []
    if (data.office_contributors && data.office_contributors.length > 0) {
      data.office_contributors.forEach(element => {
        self.model.office_contributors_param.push({
          id: element.user_type == 1 ? element.users.id : element.legal_entity.id,
          name: element.user_type == 1 ? (element.users.name + " " + (element.users.first_name ? element.users.first_name + ' ' : '') + (element.users.second_name ? element.users.second_name : ''))
            : (element.legal_entity.comm_name),
        ...
        this.model.office_contributors.push({ user_type: element.user_type, users_id: element.user_type == 1 ? element.users.id : element.legal_entity.id, office_id: data.id });
      });
    }
    this.file1.image = this.model.main_image;
    this.projectLogo.image = this.model.office_logo;
  ...
  addNewTower() {
    ...
    this.model.office_towers.push(this.newTower);
  ...
  deleteTower(index: number) {
    swal({
      ...
      if (result.value) {
        const btid = this.model.office_towers[index].id;
        this.model.office_towers.splice(index, 1);

        if (btid) {
          this.admin.postDataApi('deleteTower', { office_towers_id: btid }).subscribe(res => {
            // console.log('sss', res);
          });
        }
      }
  ...
  saveTower(btower: Towers, index: any) {
    ...
    this.model.office_towers[index].launch_date = btower.launch_date;

    // this.allTowerAmenityForEdit = btower.amenities;
    if (!this.model.office_towers[index].tower_name) {
      swal(this.translate.instant('swal.error'), this.translate.instant('message.error.pleaseEnterTowerName'), 'error');
      return false;
    }
    if (!this.model.office_towers[index].num_of_floors) {
      swal(this.translate.instant('swal.error'), this.translate.instant('message.error.pleaseEnterNumberoffloors'), 'error');
      return false;
    }
    if (!this.model.office_towers[index].possession_status_id) {
      swal(this.translate.instant('swal.error'), this.translate.instant('message.error.pleaseChoosePossessionStatus'), 'error');
      return false;
    }
    ...
    // launch date to be mandatory possession_status == presale
    if (this.model.office_towers[index].possession_status_id &&
      (this.model.office_towers[index].possession_status_id.toString() === this.apiConstants.possession_status_id) &&
      !this.model.office_towers[index].launch_date) {
      swal(this.translate.instant('swal.error'), this.translate.instant('message.error.pleaseSelectLaunchDate'), 'error');
      return false;
    }
    ...
    this.model.office_towers[index].amenitiesId = this.selectedTowerAmenitiesId;
    this.model.office_towers[index].amenities = this.selectedTowerAmenityObj;
    if (this.model.office_towers[index].amenities.length < 1) {
      // swal(this.translate.instant('swal.error'), 'Please choose tower amenities.', 'error'); return false;
      this.model.office_towers[index].amenities = [];
    ...
    this.model.office_towers[index].amenitiesCount = this.model.office_towers[index].amenities.length;
    this.model.office_tower_edit_index = '-1';
  }
  ...
  setTowerAmenity(a: any, m: any) {
    ...
    this.model.office_towers[this.towerAmenityIndex].amenities =
    ...
    this.model.office_towers[this.towerAmenityIndex].amenitiesId =
      this.allTowerAmenityForEdit.filter(op => {
        if (op.selected === true) {
          return op;
        }
      });
    this.model.office_towers[this.towerAmenityIndex].amenitiesCount = this.model.office_towers[this.towerAmenityIndex].amenities.length;
  }
  ...
  goBack = (isForBack: boolean): void => {
    if (isForBack) {
      this.router.navigate(['dashboard/office/view-office', { for: 'back' }]);
    } else {
      this.router.navigate(['dashboard/office/view-office']);
    }
  }
  ...
  selectContributor(keyword: string, type: number, dailogOpen: boolean) {
    ...
    this.admin.postDataApi('getAllBuyers', input).subscribe(r => {
      ...
      self.users.forEach(element => {
        self.model.office_contributors_param.forEach(element1 => {
  ...
  changeStatusPopUp(property_id: any, user_id: number, name: string, status: number, user: any, event: any) {
    ...
    if (event.target.checked) {
      ...
      this.model.office_contributors = this.model.office_contributors ? this.model.office_contributors : [];
      this.model.office_contributors_param = this.model.office_contributors_param ? this.model.office_contributors_param : [];
      if (this.model.office_contributors && this.model.office_contributors.length > 0) {
        let data = this.model.office_contributors.find(element => element.users_id == user_id);
        if (!data) {
          this.model.office_contributors.push({ user_type: status, users_id: user_id, office_id: property_id });
        }
      }
      else {
        this.model.office_contributors.push({ user_type: status, users_id: user_id, office_id: property_id });
      }
      if (this.model.office_contributors_param && this.model.office_contributors_param.length > 0) {
        let data = this.model.office_contributors_param.find(element => element.id == user_id)
        if (!data) {
          this.model.office_contributors_param.push({
            id: user.id,
            name: name,
            phone: user.phone,
            email: user.email,
      ...
      let selectedUser = this.model.office_contributors.findIndex(user => user.users_id == user_id);
      this.model.office_contributors.splice(selectedUser, 1);
      let UserIndex = this.model.office_contributors_param.findIndex(user => user.id == user_id);
      this.model.office_contributors_param.splice(UserIndex, 1);
    }
  }
  ...
  removeContributor(userId, index) {
    ...
    let selectedUser = this.model.office_contributors.findIndex(user => user.users_id == userId);
    if (selectedUser >= 0) {
      this.model.office_contributors.splice(selectedUser, 1);
    }
    let UserIndex = this.model.office_contributors_param.findIndex(user => user.id == userId);
    if (UserIndex >= 0) {
      this.model.office_contributors_param.splice(UserIndex, 1);
    }
  }
  ...
  getParkingLotSpaces() {
    this.spinner.show();
    this.parameter.url = 'getOfficeParkingspace';
    const input = new FormData();
